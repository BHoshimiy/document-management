...
Domain hierarchy: `Menu` (a standard, e.g. Global GAP) → `DocumentFolder` (has a `code` like `RP-FER-01`) → `Document`, tagged with a `Category`. Each client `User` has one `Company`; documents belong to a company.
